...
function MealRow({ meal }) {
  ...
  return (
    <div className="bg-gray-800/40 border border-gray-700 rounded-xl overflow-hidden transition-all hover:border-teal-500/40">
      <button
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-white">{meal.type}</p>
            <p className="text-xs text-gray-400 mt-0.5">{meal.food}</p>
          </div>
        </div>
      ...
      {expanded && (
        <div className="px-4 pb-4 pt-0 flex flex-wrap gap-2 border-t border-gray-700/50 pt-3">
          <MacroBadge label="Carbs" value={meal.carbs} unit="g" color="bg-blue-500/20 text-blue-400" />
          <MacroBadge label="Protein" value={meal.protein} unit="g" color="bg-red-500/20 text-red-400" />
          <MacroBadge label="Fat" value={meal.fat} unit="g" color="bg-yellow-500/20 text-yellow-400" />
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="p-4 md:p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-white">Meal Dashboard</h1>
          <p className="text-sm text-gray-400 mt-1">View your daily food log and macro split.</p>
        </div>
        <div className="flex items-center gap-2 bg-gray-800 border border-gray-700 rounded-lg px-3 py-2">
          ...
          <input
            ...
            className="bg-transparent text-white text-sm focus:outline-none cursor-pointer [color-scheme:dark]"
          />
        </div>
      </div>
